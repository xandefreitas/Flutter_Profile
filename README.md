...
The app follows a handful of conventions to stay usable with a screen reader, at larger system font sizes, and with "reduce motion" enabled:

- **Semantic labels on icon-only controls.** Every icon-only tappable widget (social/contact icons in the drawer, the floating deposition write button, delete/edit pencils, the certificates search field's clear button, the PDF share icon, ...) is wrapped in `Semantics(button: true, label: ...)`, or exposes a `tooltip` where the underlying widget already supports one (`IconButton`). `CustomIconButton` (`lib/common/widgets/custom_icon_button.dart`) takes this as a required `semanticLabel` parameter so every caller has to supply one.
- **Persistent labels on form fields, not just `hintText`.** A field's `hintText` disappears once the user types something, so a screen reader loses track of what the field is for. Instead of relying on `InputDecoration.label` (its floating-label layout didn't match this app's compact, borderless field styling), fields keep their visual `hintText` and get an extra `Semantics(label: ...)` wrapper around the `TextField`/`TextFormField`/`DropdownButtonFormField`, so the label is announced regardless of whether the field is empty or filled.
- **48x48 minimum tap targets.** Bare `GestureDetector`/`InkWell`-wrapped icons smaller than that are wrapped in `SizedBox(width: 48, height: 48, child: Center(child: Icon(...)))` inside the tap handler, so the hit area grows without changing the icon's visible size.
- **Toggle/expand state exposed via `Semantics`**, not just color or an icon swap — for example the skill chip's `selected: widget.skill.isRecommended` and the certificate card's `expanded: _isExpanded`.
- **`maxLines`/`overflow: TextOverflow.ellipsis` on fixed-size text containers** wherever a larger system font size could otherwise clip or overflow a label — the app doesn't cap the system font scale, so this is the safety net for short strings (titles, names) that a container was sized for at the default scale. Text that holds live user input (the deposition message field) instead has its fixed height removed entirely, relying on the surrounding `SingleChildScrollView` rather than truncating what someone typed.
- **`MotionUtil.reduceMotion(context)`** (`lib/common/util/motion_util.dart`) reads the OS "reduce motion" setting via `MediaQuery.disableAnimations`. Every looping animation — the deposition button's shake and all `Lottie.asset` animations — checks it before looping, so it plays once (or not at all) instead of repeating indefinitely for users who've asked the system not to.

A few widgets also have `meetsGuideline(...)` tests (`flutter_test`'s built-in accessibility matchers — tap target size, labeled tap targets, text contrast) alongside their regular widget tests, e.g. `test/widget/custom_icon_button_test.dart` and `test/widget/custom_screen_test.dart`. These render the widget and sample the actual pixels, which is worth knowing if you change a color or a background gradient — the numbers don't always match what hand-computed contrast math would suggest.
